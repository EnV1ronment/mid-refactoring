...
function defaultTask(cb) {
  // place code for your default task here
    ...
    exists(path.join(__dirname, './src/models'), path.join(__dirname, '../ems-view2/src/models'), copy);
    // 文件变化
    watcher.on('change',function(filepath, stats){
      exists(path.join(__dirname, filepath), path.join(__dirname, filepath.replace('src','../ems-view2/src')), copy)
    })
    // 新增文件
    ...
    watcher.on('unlink',function(filepath, stats) {
      ...
      const copyPath = path.join(__dirname, filepath.replace('src','../ems-view2/src'));
