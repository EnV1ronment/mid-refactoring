export = {
  getServerRootUrl: function (client: any, id: any) {
    return this.getRootUrlByVipAdress(client, id);
  },
  getRootUrlByVipAdress: function (client: any, vipAddress: any) {
  ...
  getOneInstanceFromAll: function (client: any, instances: any) {
    if (instances !== null) {
      var upInstances = [];
      ...
      if (upInstances.length > 0) {
        var instanceIndex = upInstances.length === 1 ? 0 : Date.now() % (upInstances.length - 1);
   ...
   */
  getServerPath: function (instance: any) {
    var url = "",
      http = "http://",
      https = "https://";
    if (instance) {
      if (instance.port && instance.port["@enabled"] === "true") {
        ...
        url = https + instance.hostName + ":" + instance.securePort["$"];
      }
    }
    return url;
  }
